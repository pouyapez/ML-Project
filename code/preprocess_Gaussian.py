...
for i in range(len(output)): # produce convolutional decoder and add Gaussian noise 
	S = [0, 0]
	c = [[0, 0]]
	inp = []
	tr_out = []
	...
	for j in range(len(output[0])):
		...
		tr_out += [[c[0][0]+np.random.normal(0, SNR), c[0][1]+np.random.normal(0, SNR)]] 
		te_out += [[c[0][0]+np.random.normal(0, SNR), c[0][1]+np.random.normal(0, SNR)]]
	input += [inp]
	train_out += [tr_out]
	test_out += [te_out]
